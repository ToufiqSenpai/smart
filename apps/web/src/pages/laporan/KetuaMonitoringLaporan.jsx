...
function getStatusInfo(status) {
  const map = {
    PENDING: { class: 'bg-warning-bg text-warning border-warning/10', label: 'Menunggu', icon: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15.5 14"/></svg>' },
    VERIFIED: { class: 'bg-teal-bg text-teal border-teal/10', label: 'Diverifikasi', icon: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 12 2 2 4-4"/></svg>' },
    IN_PROGRESS: { class: 'bg-teal-bg text-teal border-teal/10', label: 'Dalam Proses', icon: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v6l3 2"/></svg>' },
    COMPLETED: { class: 'bg-success-bg text-success border-success/10', label: 'Selesai', icon: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 12 2 2 4-4"/></svg>' },
    REJECTED: { class: 'bg-error-bg text-error border-error/10', label: 'Ditolak', icon: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="5" x2="19" y2="19"/><line x1="19" y1="5" x2="5" y2="19"/></svg>' }
  }
...
export default function KetuaMonitoringLaporan() {
  ...
  const filtered = data.filter((item) => {
    ...
    const matchStatus = statusFilter === "all" || item.status_laporan === (statusReverseMap[statusFilter] || "")
    return matchSearch && matchStatus
  })

  const total = data.length
  const menunggu = data.filter(i => i.status_laporan === 'PENDING').length
  const diverifikasi = data.filter(i => i.status_laporan === 'VERIFIED').length
  const proses = data.filter(i => i.status_laporan === 'IN_PROGRESS').length
  const selesai = data.filter(i => i.status_laporan === 'COMPLETED').length
  const ditolak = data.filter(i => i.status_laporan === 'REJECTED').length
      ...
      <div className="bg-bg-card border border-border-subtle rounded-[20px] p-4 px-6 flex items-center justify-between gap-4 flex-wrap mb-8 shadow-lux">
        <div className="flex items-center gap-4 flex-wrap flex-1">
          <div className="flex-1 min-w-[200px] relative">
            ...
            <input type="text" placeholder="Cari kategori atau pelapor..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full py-2 pl-[38px] pr-3 font-sans text-[13.5px] text-text-primary bg-bg border border-border-subtle rounded-full outline-none h-[38px] transition-all focus:border-primary focus:shadow-[0_0_0_3px_rgba(30,75,133,0.06)]" />
          </div>
          ...
          <table className="w-full border-collapse text-[14px]">
            ...
            <tbody id="tableBody">
              {loading ? (
                ...
                <tr><td colSpan="6" className="text-center py-12 text-text-muted text-[13.5px]">Tidak ada laporan.</td></tr>
                ...
                return (
                  <tr key={item.id} className="hover:bg-bg">
                    ...
                    <td className="px-4 py-3 border-b border-border-subtle text-text-muted align-middle last:border-b-0"><span className="font-semibold text-text-primary">{item.kategori_kendala}</span></td>
                    <td className="px-4 py-3 border-b border-border-subtle text-text-muted align-middle last:border-b-0">{item.pelapor}</td>
                    <td className="px-4 py-3 border-b border-border-subtle text-text-muted align-middle last:border-b-0"><span className="font-mono text-[12px] text-text-primary">{formatDate(item.tanggal_lapor)}</span></td>
