...
export default function WargaUMKMSaya() {
  ...
  var filtered = data.filter(function(u) {
    var matchSearch = u.nama_usaha.toLowerCase().includes(search.toLowerCase())
    var matchFilter = filter === "all" || u.status_verifikasi === filter
            ...
            <div key={u.id} className="bg-bg-card rounded-[20px] border border-border-subtle shadow-lux overflow-hidden hover:shadow-lg transition-all">
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <span className={"inline-flex items-center gap-1 px-3 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap shrink-0 " + statusClass[u.status_verifikasi]}>{statusLabel[u.status_verifikasi]}</span>
                </div>
                <p className="text-[13px] text-text-muted mb-1">{u.jenis_usaha}</p>
                <p className="text-[13px] text-text-secondary line-clamp-2 leading-relaxed mb-3">{u.deskripsi_usaha}</p>
