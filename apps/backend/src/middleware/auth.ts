...
export async function authenticate(
  ...
  if (!authHeader?.startsWith("Bearer ")) {
    res
      .status(401)
      .json({ message: "Token tidak valid", code: "UNAUTHORIZED" });
    return;
  }
  ...
  try {
    ...
    if (!masyarakat) {
      res
        .status(401)
        .json({ message: "Pengguna tidak ditemukan", code: "UNAUTHORIZED" });
      return;
    }
...
export function authorize(...roles: AuthUser["role"][]) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (roles.length > 0 && !roles.includes(req.user.role)) {
      res
        .status(403)
        .json({ message: "Tidak memiliki hak akses", code: "FORBIDDEN" });
      return;
    }
    next();
  };
